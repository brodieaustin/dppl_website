				...
				$(form).ajaxSubmit({
					success: function(data){
							...
							if (data.search("Thank") == 1 ){
								$('div.response').html(data).css({backgroundColor: 'green'}).slideDown(800).delay(5000);
							}
							else{
								$('#application').fadeIn(1000);
								$('div.response').html(data).css({backgroundColor: 'red'}).slideDown(800).delay(4000).slideUp(800);
							}	
	...
	function saveForm(){
		console.log('save form function');
